...
?>
	<tr<?php echo $class;?> id="<?php echo $task['Task']['id']; ?>">
		...
		<td>
			<?php echo $this->Html->link($task['Task']['name'], array('action' => 'view', $task['Task']['id']), array('escape' => false)); ?>
		</td>
		<td>
		...
		<td class="actions">
			...
			if ($this->request->params['named']['filter'] != 'completed:1') { 
				echo $this->Html->link(__('Complete'), array('action' => 'complete', $task['Task']['id']), null, sprintf(__('Are you sure you want to complete # %s?'), $task['Task']['id'])); 
			} else {
				echo $this->Html->link(__('InComplete'), array('action' => 'incomplete', $task['Task']['id'], $task['Task']['id'])); 
			}
